...
model {
 ...
 for(i in 1:n_s){
  ...
  if(pred_ind == 1){
   ...
   // mu[i] = L[state[i]]/(1.0 + exp(k_eff*(t_scale[i]-x_0[state[i]])));
   mu = L[state[i]]*inv_logit(k_eff*(t_scale[i]-x_0[state[i]]));
   be = invphi/mu;
   
   mu_ber = alpha + t_scale[i]*beta[state[i]];
   
   pred[i] = inv_logit(mu_ber)*mu;
    }
  }
...
// if(t_scale[i] >= t_0){
  ...
  //  }
   // mu[i] = L[state[i]]/(1.0 + exp(k_eff*(t_scale[i]-x_0[state[i]])));
  // pred[i] = inv_logit(alpha + t_scale[i]*beta[state[i]])*L[state[i]]*inv_logit(k_eff*(t_scale[i]-x_0[state[i]]));
}  
